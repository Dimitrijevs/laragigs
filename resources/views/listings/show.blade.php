...
<div class="mx-4">
    <x-card class="p-8">
        <div class="flex flex-col items-center justify-center text-center">
            <img class="w-48 mr-6 mb-6" src="{{ asset($listing->logo ? asset('storage/' . $listing->logo) : asset('/images/no-image.png')) }}" alt=""/>
    ...
    <x-card class="mt-4 p-2 flex space-x-6">
        ...
        <form method="POST" action="/listings/{{ $listing->id }}">
            @csrf
            @method('DELETE')
            <button type="submit"><i class="fa-solid fa-trash text-red-500"></i> Delete</button>
        </form>
    </x-card>
</div>
</x-layout>
